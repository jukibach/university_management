...
import static fpt.com.universitymanagement.common.Constant.NON_EXISTENT_USER;
...
public class AccountServiceImpl implements AccountService {
    ...
    public LoginHistory findByToken(String accessToken) {
        return loginHistoryRepository.findByToken(accessToken).orElseThrow(() -> new UsernameNotFoundException("Access token does not exist"));
    }
    ...
    private Account findAccountById(long accountId) {
        ...
        if (optional.isPresent()) {
            logger.info("User is found: {}", optional.get());
            return optional.get();
        } else {
            logger.error(NON_EXISTENT_USER);
            throw new UsernameNotFoundException(NON_EXISTENT_USER);
        }
    }
    
    private Account findByUserName(String userName) {
        return accountRepository.findByUserName(userName).orElseThrow(() -> new UsernameNotFoundException(NON_EXISTENT_USER));
    }
